...
import { v2 as cloudlinary } from 'cloudinary'
...
const updateProfile = async (req, res) => {
    try {
        ...
        if (imageFile) {
            ...
            const imageUpload = await cloudlinary.uploader.upload(imageFile.path, { resource_type: 'image' })
...
const paymentStripe = async (req, res) => {
    ...
    try {
        ...
        // creating options for strip payment
        const options = {
        ...
        const order = await stripeInstance.order.create(options);
...
const verifyStripe = async (req, res) => {
    try {
        ...
        const orderInfo = await stripeInstance.verifyMicrodepositsForSetup.fetch(stripe_order_id)
